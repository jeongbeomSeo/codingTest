...
public class Main {
  public static void main(String[] args) throws IOException {
    ...
    for(int i = 0; i < M; i++) {
      ...
      if (n1 == n2) continue;
      if (graph[n1].contains(n2)) continue;
      graph[n1].add(n2);
      graph[n2].add(n1);
    }
  ...
  static void MinsikDfs(boolean[] isVisited,  ArrayList<Integer>[] graph, int node) {
    ArrayList<Integer> notVisitedNode = new ArrayList<>();
    isVisited[node] = true;
    System.out.print(node + " ");
...
놀랍게도 ArrayList<Integer>[] 보다 ArrayList<ArrayList<Integer>>가 더 빠르다.
...
public class Main {
  ...
  static void MinsikDfs(boolean[] isVisited,  ArrayList<Set<Integer>> graph, int node) {
    ArrayList<Integer> notVisitedNode = new ArrayList<>();
    isVisited[node] = true;
    System.out.print(node + " ");

    while (true) {
      Iterator<Integer> curNodes = graph.get(node).iterator();
      while (curNodes.hasNext()) {
        int curNode = curNodes.next();
        if(!isVisited[curNode]) notVisitedNode.add(curNode);
      }
