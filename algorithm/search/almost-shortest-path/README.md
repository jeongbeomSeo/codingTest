...
public class Main {
  ...
  public static void main(String[] args) throws IOException {
    ...
    while (true) {
      ...
      List<List<Node>> trace = new ArrayList<>();
      ...
      for (int i = 0; i < M; i++) {
        ...
        graph.get(src).add(new Node(src, dst, cost));
      }

      int[] table = initTable(N, start);

      dfs(graph, trace, table, start, end);

      removeShortestPaths(graph, trace, end);

      int[] newTable = initTable(N, start);

      dfs(graph, newTable, start, end);

      bw.write((newTable[end] != INF ? newTable[end] : -1) + "\n");
    }
    bw.flush();
    bw.close();
  }
  private static void removeShortestPaths(List<List<Node>> graph, List<List<Node>> trace, int end) {

    for (int i = 0; i < trace.get(end).size(); i++) {
      Node pathNode = trace.get(end).get(i);

      graph.get(pathNode.src).remove(pathNode);
    }
  }
  private static void dfs(List<List<Node>> graph, int[] table, int start, int end) {
    dfs(graph, null, table, start, end);
  }
  private static void dfs(List<List<Node>> graph, List<List<Node>> trace, int[] table, int start, int end) {
    ...
    while (!pq.isEmpty()) {
      ...
      if (table[curNode.dst] < curNode.cost) {
        continue;
      }

      for (int i = 0; i < graph.get(curNode.dst).size(); i++) {
        Node nxtNode = graph.get(curNode.dst).get(i);

        if (table[nxtNode.dst] == table[curNode.dst] + nxtNode.cost) {
          if (trace != null) {
            registTrace(trace, curNode.dst, nxtNode.dst, nxtNode);
          }
        }

        if (table[nxtNode.dst] > table[curNode.dst] + nxtNode.cost) {
          table[nxtNode.dst] = table[curNode.dst] + nxtNode.cost;

          pq.add(new Node(nxtNode.src, nxtNode.dst, table[curNode.dst] + nxtNode.cost));
          if (trace != null) {
            if (!trace.get(nxtNode.dst).isEmpty()) {
              trace.get(nxtNode.dst).clear();
            }
            registTrace(trace, curNode.dst, nxtNode.dst, nxtNode);
          }
        }
  ...
  private static void registTrace(List<List<Node>> trace, int src, int dst, Node nxtNode) {

    for (int i = 0; i < trace.get(src).size(); i++) {
      trace.get(dst).add(trace.get(src).get(i));
    }

    trace.get(dst).add(nxtNode);
  }
...
class Node {
  ...
  public boolean equals(Object obj) {
    if (obj instanceof Node) {
      Node o = (Node) obj;
      return this.dst == o.dst && this.cost == o.cost;
    }
    return false;
  }
}
...
public class Main {
  ...
  private static void updateTrace(ArrayList<Integer>[] trace, int startIdx, int endIdx) {
    ...
    for (int i = 0; i < trace[startIdx].size(); i++) {
      trace[endIdx].add(trace[startIdx].get(i));
    }
...
정말 많이 시도해보고 찾아보고 하다가 메모리 초과의 원인을 해결한 것 같다.

문제의 원인은 모든 Tracking 정보를 다음 노드로 전달하는 것이 문제였습니다.

다익스트라 알고리즘의 주요 핵심 특징 중 하나는 해당 경로를 방문하는 노드에서 업데이트가 되거나 최단 경로와 같은 cost를 갖는 노드의 방문외에는 다른 노드는 방문하지 않는다는 것입니다.

좀 더 쉽게 풀어서 얘기하자면, 방문한 노드중에 이전 노드만 기록해놓고 End Point Node부터 방문한 노드를 순차적으로 탐색한다면 Start Point Node에 도달할 수 있다는 것입니다.
...
**이번 건 진짜 포기할 뻔 했네요 원인 찾기가 너무 어려웠네요 ,,,,**
