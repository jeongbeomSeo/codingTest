...
해당 문제는 구슬 탈출 2의 문제와 유사하기 떄문에 코드와 반례 위주로 적겠습니다.
        ...
        while (!q.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (isSamePoint(nextRedBallPoint, nextBlueBallPoint)) {
                    if (priorityCheck(initRedBallPoint, initBlueBallPoint, i)) {
...
해당 코드에서 init위치를 사용한 것이 문제다. `curStatus`의 ballPoint를 사용해야되는데 잘못 사용했다.

이전에도 이런 실수를 했는데 다시 한번 주의하자. 
...
이전 내 로직의 문제는 blueBall이 구멍에 들어갈 경우 무조건 해당 경우는 실패한 것인데, 

redBall이 들어간 경우에만 blueBall이 구멍에 들어갔는지 체크하는 바람에 blueBall만 구멍에 들어간 경우에 추가적으로 계속 탐색을 하는 문제가 생긴다.
